/*큰 수 합 : 0~10^10000 사이의 정수의 합을 구하는 코드*/
...
stack<int> calcPlus(string& a, string& b) {
    ...
    while (p1 >= 0) { //a에서 남은 숫자 반영
        ...
        num += carry;           //올림값 더함
        carry = num / 10;       //자릿수 더한 값이 10을 넘으면 올림변수 체크
        st.push(num % 10);      //올림값 제회한 값만 스택에 저장
    }
    if (carry) { //마지막 올림 확인
        st.push(1); // 올림이 있을 경우 마지막자리수에 1 추가
    }
...
int main() {
    ...
    //출력
    while (!st.empty()) {
        cout << st.top();   //stack 앞부분 : 큰자리수 -> 큰자리수부터 출력
        st.pop();
    }
    cout << '\n';
