...
int minusPosition(int n, int pos) { //인덱스 감소
    if (--pos >= 0) {   //오른쪽으로 회전 -> 자리수 1 감소
        return pos;
    }
    return n * 2 - 1;   //음수 나올 경우 +2n 해줘서 2n - 1
}

void second(int n, int start, int end) {    //2단계
    int cur = end;
    while (cur != start) { //end - 1 부터 start 까지 검사
        ...
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구성 1이상인 경우 -> 로봇 옮김
            ...
            belt[next].power--;     //로봇 옮겨진 벨트 내구도 1 감소
            if (next != end) { //내리는 위치 아닐 경우만
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) {    //내구도 0될 경우
                zero_power++;   //내구도 0인 벨트 수 1 증가
            }
        }
    }
}

void third(int start) { //3단계
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올ㄱㅖ
        belt[start].is_on = true;   //시작 벨트에 로봇 있으면
        belt[start].power--;        //내구도 1 감소
        if (belt[start].power == 0) {   //내구도가 0될 경우
            zero_power++;   //내구도가 0된 로봇 갯수 1 증가
        }
    }
}
...
int main() {
    ...
    int step = 0;     //단계 수
    while (true) {
        start = minusPosition(n, start);    //오른쪽으로 회전 시 바뀌는 start지점
        end = minusPosition(n, end);        //오른쪽으로 회전 시 바뀌는 end지점
        if (belt[end].is_on) { //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false;    //로봇 내렸으니까 false로 바꾸기
        }
        second(n, start, end);  //2
        third(start);   //3
        step++;     //단계 수 증가
        if (zero_power >= k) {  //내구도 0이 k개보다 커지면
            break;  //증단
        }
    }
